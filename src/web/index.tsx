...
import { JenkinsView } from './jenkins/JenkinsView.tsx'
import { TransferQueueBar } from './file-transfer/TransferQueueBar.tsx'
import './web.css'
import './pro-max.css'

const TabIcon = ({ kind }: { kind: 'topology' | 'agent' | 'files' | 'ssh' | 'ci' }): JSX.Element => {
  const paths = {
    ...
    ci: <><path d="M21 12a9 9 0 1 1-3-6.7"/><path d="M21 3v6h-6"/><path d="M12 8v4l3 2"/></>,
  }
...
function WebApp(): JSX.Element {
  ...
  return <div className="web-shell"><StyleInjector /><a className="web-skip-link" href="#web-main">跳到主要内容</a><header className="web-header"><div className="web-header-brand"><span className="web-brand-mark small">L</span><div><span className="web-kicker">SONiC LAB / DEVICE MANAGEMENT</span><h1>设备管理</h1></div></div><div className="web-user"><span className="web-live"><i />服务在线</span><code>{user}</code><button onClick={() => { void logout() }}>退出</button></div></header><nav className="web-tabs" aria-label="工作区标签"><button className={activeTab === 'topology' ? 'active' : ''} onClick={() => setActiveTab('topology')}><span className="web-tab-icon"><TabIcon kind="topology" /></span>物理拓扑</button><button className={activeTab === 'agent' ? 'active terminal' : 'terminal'} onClick={() => setActiveTab('agent')}><span className="web-tab-icon"><TabIcon kind="agent" /></span>Agent 工作台</button><button className={activeTab === 'files' ? 'active terminal' : 'terminal'} onClick={() => setActiveTab('files')}><span className="web-tab-icon"><TabIcon kind="files" /></span>文件传输</button><button className={activeTab === 'ci' ? 'active terminal' : 'terminal'} onClick={() => setActiveTab('ci')}><span className="web-tab-icon"><TabIcon kind="ci" /></span>CI 构建</button>{sshTabs.map((tab) => <div key={tab.id} className={activeTab === tab.id ? 'web-tab-item active' : 'web-tab-item'}><button className="web-tab-main terminal" onClick={() => setActiveTab(tab.id)}><span className="web-tab-icon"><TabIcon kind="ssh" /></span>{tab.switchId.toUpperCase()} SSH {tab.index}<span className={'web-tab-status ' + (sshStatus[tab.id] ?? 'connecting')} title={sshStatus[tab.id] === 'connected' ? '已连接' : sshStatus[tab.id] === 'error' ? '连接错误' : sshStatus[tab.id] === 'closed' ? '已断开' : '连接中'} /></button><button className="web-tab-close" aria-label={'关闭并断开 '+tab.switchId.toUpperCase()+' SSH '+tab.index} onClick={() => closeSsh(tab.id)}>×</button></div>)}</nav><main id="web-main" className="web-workspace"><section className={activeTab === 'topology' ? 'web-panel active' : 'web-panel'}><LabView visible={activeTab === 'topology'} showExperiment={false} currentUsername={user} onOpenSsh={openSsh} onAddSsh={addSsh} sshInstanceCounts={sshInstanceCounts} /></section><section className={activeTab === 'agent' ? 'web-panel active' : 'web-panel'}><AgentView /></section><section className={activeTab === 'files' ? 'web-panel active web-panel-files' : 'web-panel web-panel-files'}><FileTransferView /><TransferQueueBar /></section><section className={activeTab === 'ci' ? 'web-panel active' : 'web-panel'}><JenkinsView visible={activeTab === 'ci'} /></section>{sshTabs.map((tab) => <section key={tab.id} className={activeTab === tab.id ? 'web-panel active' : 'web-panel'}><SshTerminal switchId={tab.switchId} onConnectionChange={(state) => updateSshStatus(tab.id, state)} /></section>)}</main></div>
}
