...
function WebApp(): JSX.Element {
  ...
  return <div className="web-shell"><StyleInjector /><header className="web-header"><div className="web-header-brand"><span className="web-brand-mark small">L</span><div><span className="web-kicker">SONiC LAB / DEVICE MANAGEMENT</span><h1>设备管理</h1></div></div><div className="web-user"><span className="web-live"><i />服务在线</span><code>{user}</code><button onClick={() => { void logout() }}>退出</button></div></header><nav className="web-tabs"><button className={activeTab === 'topology' ? 'active' : ''} onClick={() => setActiveTab('topology')}><span className="web-tab-icon">⌘</span>物理拓扑</button><button className={activeTab === 'agent' ? 'active terminal' : 'terminal'} onClick={() => setActiveTab('agent')}><span className="web-tab-icon">◇</span>Agent 工作台</button><button className={activeTab === 'files' ? 'active terminal' : 'terminal'} onClick={() => setActiveTab('files')}><span className="web-tab-icon">⇄</span>文件传输</button>{sshTabs.map((tab) => <button key={tab.id} className={activeTab === tab.id ? 'active terminal' : 'terminal'} onClick={() => setActiveTab(tab.id)}><span className="web-tab-icon">›_</span>{tab.switchId.toUpperCase()} SSH {tab.index}<span className={'web-tab-status ' + (sshStatus[tab.id] ?? 'connecting')} title={sshStatus[tab.id] === 'connected' ? '已连接' : sshStatus[tab.id] === 'error' ? '连接错误' : sshStatus[tab.id] === 'closed' ? '已断开' : '连接中'} /><i onClick={(event) => { event.stopPropagation(); closeSsh(tab.id) }} title="关闭并断开 SSH">×</i></button>)}</nav><main className="web-workspace"><section className={activeTab === 'topology' ? 'web-panel active' : 'web-panel'}><LabView visible={activeTab === 'topology'} showExperiment={false} onOpenSsh={openSsh} onAddSsh={addSsh} sshInstanceCounts={sshInstanceCounts} /></section><section className={activeTab === 'agent' ? 'web-panel active' : 'web-panel'}><AgentView /></section><section className={activeTab === 'files' ? 'web-panel active' : 'web-panel'}><FileTransferView /></section>{sshTabs.map((tab) => <section key={tab.id} className={activeTab === tab.id ? 'web-panel active' : 'web-panel'}><SshTerminal switchId={tab.switchId} onConnectionChange={(state) => updateSshStatus(tab.id, state)} /></section>)}</main></div>
}
