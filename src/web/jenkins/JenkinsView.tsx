...
export function JenkinsView({ visible }: { visible: boolean }): JSX.Element {
	...
	return <div className="jk-root">
		...
		{Object.values(genError).some((msg)=>msg.includes('模型')||msg.includes('Mock'))&&<div className="jk-model-hint" role="alert">分析模型未配置:请通过 PUT /api/agent/me/provider 接口配置 DeepSeek 或 OpenAI 兼容模型与 API Key(或联系管理员),完成后回到本页点击「重试」即可批量生成历史失败报告。</div>}
