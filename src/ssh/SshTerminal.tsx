...
export function SshTerminal({ switchId, onConnectionChange }: { switchId: string; onConnectionChange?: (state: SshConnectionState) => void }): JSX.Element {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => { const message = JSON.parse(String(event.data)) as { type: string; data?: string; message?: string }; if (message.type === 'data') terminal.write(message.data ?? '', () => terminal.scrollToBottom()); if (message.type === 'error') { onConnectionChange?.('error'); terminal.writeln('\r\n\x1b[31m' + (message.message ?? 'SSH error') + '\x1b[0m') } }
