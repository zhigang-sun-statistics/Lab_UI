...
export function SshTerminal({ switchId }: { switchId: string }): JSX.Element {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => { const message = JSON.parse(String(event.data)) as { type: string; data?: string; message?: string }; if (message.type === 'data') terminal.write(message.data ?? '', () => terminal.scrollToBottom()); if (message.type === 'error') terminal.writeln('\r\n\x1b[31m' + (message.message ?? 'SSH error') + '\x1b[0m') }
