...
const api = async (req: IncomingMessage, res: ServerResponse, path: string): Promise<boolean> => {
  ...
  // Agent workbench (jobs/messages/attachments) removed from the web build;
  // the provider route stays as the model-configuration API for CI analysis.
  ...
  if (path === '/api/files/me/file' && req.method === 'GET') { const url = new URL(req.url ?? '/', 'http://localhost'); json(res, 200, await readRemoteText(lab, url.searchParams.get('switch') ?? '', url.searchParams.get('path') ?? '')); return true }
  if (path === '/api/files/me/file' && req.method === 'PUT') { const input = await body(req); if (typeof input.switchId !== 'string' || typeof input.path !== 'string' || typeof input.content !== 'string') { json(res, 400, { error: 'switchId, path and content are required' }); return true } json(res, 200, await writeRemoteText(lab, input.switchId, input.path, input.content)); return true }
